...
class JobManager:
    ...
    async def handle_ingest_raw(self, raw: str) -> None:
        try:
            msg = json.loads(raw)
    ...
    def _next_ready_item(self) -> dict | None:
        """First pending item, unless it's scheduled for later — then hold the queue."""
        now = time.time()
        for q in self._queue:
    ...
    async def _scheduler_task(self) -> None:
        """Launch the next ready queue item whenever the manager is idle."""
        while True:
