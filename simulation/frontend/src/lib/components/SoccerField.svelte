<script lang="ts">
	...
	let {
		...
		showNeutralSpots = true,
		// ── Colors (RAL approximations per the official rules) ────────────────
		wallColor = '#1a1a1a',
		neutralColor = '#8c8c8c',
		grassColor = '#009C3B',        // playfield green ≈ RAL 6038
		lineColor = 'white',
		penaltyColor = '#0a0a0a',      // strafschopgebied black ≈ RAL 9005
		ballColor = '#ffa000',
		ownGoalColor = '#2271B3',      // goal blue ≈ RAL 5015
		enemyGoalColor = '#E1CC4F',    // goal yellow ≈ RAL 1016
		neutralSpotColor = 'white',
		allyColor,
		enemyColor,
		// ── Dimensions (mm) ──────────────────────────────────────────────────
		robotRadius = 110,             // 220 mm diameter (official max)
		ballRadius = 21,
		fieldHalfWidth = 610,
		fieldHalfHeight = 915,
		neutralZone = 300,
	...
	// The 5 neutral spots (rules §1.4): the inner (field-side) corner of each penalty
	// area plus the centre spot. Used to reset the ball / re-enter robots.
	let NEUTRAL_PTS = $derived([
		{ x: 0, y: 0 },
...
-->
<div
	class="overflow-hidden border border-border {fit
...
>
	<!--
        ...
        In fit mode the svg fills its box and preserveAspectRatio (meet) letterboxes.
    -->
	<svg
			viewBox="0 0 {vbW} {vbH}"
			class="block {fit ? 'h-full w-full' : 'w-full h-auto'}"
			role="img"
	...
	>
		...
		<g transform="translate({CW2}, {CH2}) scale(1, -1)">
			...
			<!-- Neutral spots (rules §1.4): penalty-area inner corners + centre -->
			{#if showNeutralSpots}
				{#each NEUTRAL_PTS as p}
					<circle
							cx={p.x} cy={p.y} r="22"
							fill="none" stroke={neutralSpotColor} stroke-width="4"
							stroke-dasharray="10 7" opacity="0.85"
					/>
					<circle cx={p.x} cy={p.y} r="5" fill={neutralSpotColor} opacity="0.85" />
				{/each}
			{/if}
