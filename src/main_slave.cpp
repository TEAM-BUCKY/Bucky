...
void loop()
{
  ...
  else
  {
    ...
    if (-5 < left.read() - leftSum && left.read() - leftSum < 5 || right.read() - rightSum > -5 && right.read() - rightSum < 5) {
        move(0, 100, magnetomer.calculate());
    }
    else if (-5 > left.read() || 5 > right.read()){
      move(90, 100, magnetomer.calculate());
      
    }
    else if (-5 > right.read() || 5 > left.read()) {
      move(-90, 100, magnetomer.calculate());
    }
    else {
      move(0, 255, 0);
