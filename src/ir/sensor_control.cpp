...
void setAllSensorPinsInput() {
    for (uint8_t SensorPin : SensorPins) {
...
sensorInfo_t getAllSensorPulseWidth(int pulseWidth[IR_NUM], uint16_t timeLimit) {
    ...
    while((micros() - startTime_us) < timeLimit) {
        for (int i = 0; i < IR_NUM; i++) {
            if(getSensorPin(i)) {
                pulseWidth[i] += micros() - startTime_us; // instead of pulseWidth[i] += deltaPulseWidth;
            }
        }
    };
    ...
    for(int i = 0; i < IR_NUM; i++) {
        ...
        if(pulseWidth[i] > sensorInfo.maxPulseWidth) {
            sensorInfo.maxPulseWidth = (int)pulseWidth[i];
            sensorInfo.maxSensorNumber = i;
            Serial.println("maxSensorNumber: " + String(sensorInfo.maxSensorNumber));
        }
    }
...
vectorXY_t calcVectorXYFromPulseWidth(const int pulseWidth[IR_NUM]) {
    vectorXY_t rslt = {0, 0};
    for (int i = 0; i < IR_NUM; i++) {
//        rslt.x += pulseWidth[i] * unitVectorX[i];
//        rslt.y += pulseWidth[i] * unitVectorY[i];
        rslt += vectorXY_t{pulseWidth[i] * unitVectorX[i], pulseWidth[i] * unitVectorY[i]};
    }
    return rslt;
}
